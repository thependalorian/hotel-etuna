...
describe('Database - Tenant Operations', () => {
  ...
  it('should enforce unique slug constraint', async () => {
    const testSlug = `unique-slug-${Date.now()}`;
...
describe('Database - Multi-Tenancy', () => {
  ...
  beforeAll(async () => {
    ...
    const [t1] = await db
      .insert(tenants)
      .values({ name: 'Tenant 1', slug: `tenant1-${Date.now()}` })
      .returning();
    tenant1Id = t1.id;

    const [t2] = await db
      .insert(tenants)
      .values({ name: 'Tenant 2', slug: `tenant2-${Date.now()}` })
      .returning();
    tenant2Id = t2.id;
...
describe('Database - Complex Queries', () => {
  ...
  beforeAll(async () => {
    ...
    const [tenant] = await db
      .insert(tenants)
      .values({ name: 'Complex Test Tenant', slug: `complex-${Date.now()}` })
      .returning();
    testTenantId = tenant.id;
...
describe('Database - Sofia AI Conversations', () => {
  ...
  beforeAll(async () => {
    const [tenant] = await db
      .insert(tenants)
      .values({ name: 'Sofia Test Tenant', slug: `sofia-${Date.now()}` })
      .returning();
    testTenantId = tenant.id;
  });
...
describe('Database - Performance & Indexing', () => {
  ...
  it('should handle bulk inserts efficiently', async () => {
    const [tenant] = await db
      .insert(tenants)
      .values({ name: 'Bulk Test', slug: `bulk-${Date.now()}` })
      .returning();
