...
async function main() {
  ...
  securityLogger.info(`Seeding menu thumbnails (${MENU_IMAGE_THUMB_WIDTH}px wide)…`);
  if (force) securityLogger.info('  --force: overwriting all image_url values');
  ...
  securityLogger.info(`Catalog image map: ${catalogUrls} items`);
  securityLogger.info(`DB rows updated: ${updated}`);
  securityLogger.info(`DB rows total: ${rows.length}`);
  securityLogger.info(`DB rows with image_url: ${withImage}`);
  if (skippedNoMatch > 0) {
    securityLogger.warn(`  ${skippedNoMatch} DB row(s) not in catalog (used category fallback URL)`);
  }

  if (withImage < rows.length) {
    securityLogger.warn(`  ${rows.length - withImage} row(s) still missing image_url`);
    process.exit(1);
  }
}
