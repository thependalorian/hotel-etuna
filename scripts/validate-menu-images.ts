...
function printFailures(
  ...
  for (const item of items.slice(0, 15)) {
    if (typeof item === 'string') {
      securityLogger.error(`  - ${item}`);
    } else {
      securityLogger.error(`  - ${item.name}: ${item.status ?? ''} ${item.url ?? ''}`);
    }
  }
  if (items.length > 15) securityLogger.error(`  … and ${items.length - 15} more`);
}
...
async function main() {
  ...
  securityLogger.info(`Validating menu images (expected thumb width ≤ ${MENU_IMAGE_THUMB_WIDTH}px)…`);

  const result = await validateMenuItemImages(sql, { checkHttp: !skipHttp });

  securityLogger.info(`DB: ${result.dbWithImage}/${result.dbTotal} with image_url`);
