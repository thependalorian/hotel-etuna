 ...
 *
 * Requires server env: QDRANT_URL, OPENAI_API_KEY. Sofia still needs RAG_ENABLED=true for retrieval.
 */
...
import { isOpenAiEmbeddingConfigured } from '@/lib/integrations/embeddings-openai';
  ...
  return withApiAuth(
    ...
    async (req, user) => {
      ...
      if (!isOpenAiEmbeddingConfigured()) {
        return errorResponse(
          'OpenAI embeddings are not configured (OPENAI_API_KEY)',
          503,
