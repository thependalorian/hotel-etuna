...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const result = await securityService.validatePaymentSecurity({
      ...
      ipAddress: validatedData.ipAddress || req.headers.get('x-forwarded-for') || req.headers.get('x-real-ip') || undefined,
      userAgent: validatedData.userAgent || req.headers.get('user-agent') || undefined,
...
export async function GET(req: NextRequest) {
  try {
    ...
    // Get statistics
    const stats = await securityService.getSecurityStatistics(tenantId, days);
    const complianceRate = await securityService.checkPSD12ComplianceRate(tenantId);
    const highRiskTxns = await securityService.getHighRiskTransactions(tenantId, 20);
