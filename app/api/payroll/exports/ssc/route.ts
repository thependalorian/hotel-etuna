 ...
 *
 * Returns text/csv for Social Security remittance (approved run lines).
 */
...
export async function GET(request: NextRequest) {
  return withApiAuth(
    ...
    async (req, user) => {
      ...
      try {
        const csv = await payrollService.exportSscCsv(user.tenantId, periodId);
        return new NextResponse(csv, {
          status: 200,
          headers: {
            'Content-Type': 'text/csv; charset=utf-8',
            'Content-Disposition': `attachment; filename="ssc-${periodId}.csv"`,
