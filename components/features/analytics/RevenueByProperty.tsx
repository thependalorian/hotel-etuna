...
export default function RevenueByProperty({ revenueByProperty }: RevenueByPropertyProps) {
  return (
    <div className="dashboard-card p-6 animate-slide-up">
      <h2 className="font-display text-xl font-bold text-nude-900 mb-4">Revenue Breakdown</h2>
      <div className="space-y-3">
        {revenueByProperty.map((item, index) => (
          ...
          >
            <div>
              <p className="font-medium text-nude-900">{item.propertyName}</p>
              <p className="text-sm text-nude-600">
                {item.percentage.toFixed(1)}% of total
              </p>
            </div>
            <div className="text-right">
              <p className="font-bold text-nude-900">N${item.revenue.toLocaleString()}</p>
            </div>
          </div>
        ))}
        {revenueByProperty.length === 0 && (
          <p className="text-nude-600 text-center py-4">No revenue data available yet.</p>
        )}
      </div>
    </div>
  );
