        ...
        <div className="space-y-4">
          {bookings.map((booking, index) => (
            <Card 
              key={booking.id} 
              variant="elevated"
              className="animate-slide-up hover:-translate-y-0.5 transition-all duration-200"
            ...
            >
              <CardContent className="p-6">
                ...
                {(booking.roomType ?? booking.room_type) ? (
                  <p className="text-sm text-ink-600 mb-2">{booking.roomType ?? booking.room_type}</p>
                ) : null}
                
                {/* Date Information */}
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4 p-4 bg-nude-50 rounded-etuna-input border border-nude-200">
                  <div>
                    <p className="text-xs font-semibold text-ink-700 mb-1 flex items-center gap-1">
                ...
                {(booking.roomCount ?? booking.room_count ?? 0) > 0 && (
                  <div className="mb-4">
                    <div className="inline-flex items-center gap-2 px-3 py-1.5 bg-nude-100 text-ink-800 rounded-full text-sm font-medium">
