...
export async function runWithTenantContext<T>(
  ...
  tenantTypeOrFn: 'hub' | 'partner' | (() => Promise<T>),
  maybeFn?: () => Promise<T>
): Promise<T> {
  const tenantType =
    typeof tenantTypeOrFn === 'function' ? 'hub' : tenantTypeOrFn;
  const fn =
    typeof tenantTypeOrFn === 'function' ? tenantTypeOrFn : maybeFn;

  if (!fn) {
    throw new Error('runWithTenantContext requires a callback function');
  }
  ...
  try {
    await setTenantContext(tenantId, tenantType);
    return await fn();
  } finally {
    try {
