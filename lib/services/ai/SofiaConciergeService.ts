...
export class SofiaConciergeService {
  ...
  async processMessage(request: AIRequest, userRole: UserRole = 'guest'): Promise<AIResponse> {
    try {
      ...
      const context = await this.buildContext(request.context, userRole, channel, request.message);
      const aiResponse = await this.callLlmProvider(request.message, context, conversationHistory);
  ...
  private async buildContext(
    ...
    try {
      ...
      if (ragQuery) {
        const ragChunks = await this.ragSearch.search(ragQuery, context.tenantId, {
  ...
  private buildSofiaSystemPrompt(
    ...
    return `You are Sofia, an AI concierge for Buffr Host hospitality management platform in Namibia. 
ABOUT BUFFR HOST:
- Buffr Host is an enterprise-grade hospitality management platform for Namibian properties
...
- Support is available 24/7 via Sofia AI at concierge@buffr.ai
...
- Respond in the language of the user (English or Afrikaans)
