...
export default function CrmReviewsPage() {
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      try {
        ...
        const reviewsData = (await reviewsRes.json()) as ApiReview[];
  ...
  const filteredReviews = useMemo(() => {
    ...
    let result = reviews.filter((review) => {
      if (selected && review.propertyId !== selected) {
        return false;
      }
        ...
        <Card variant="elevated">
          ...
          <ReviewsTable
            reviews={filteredReviews.map((review) => ({
              id: review.id,
              guest_id: review.guestId ?? 'anonymous',
              property_id: review.propertyId ?? '',
              rating: review.rating ?? 0,
              review_text: review.reviewText ?? '',
              created_at: review.createdAt,
              is_public: Boolean(review.isPublic),
              guest_name: review.guestId ? guestNameMap[review.guestId] ?? 'Anonymous' : 'Anonymous',
              property_name: review.propertyId ? propertyNameMap[review.propertyId] ?? 'Unknown Property' : 'Unknown Property',
            }))}
