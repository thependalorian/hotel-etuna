...
This is an automated notification from the Hotel Etuna Fraud Detection System.
    `.trim();
...
<body>
  ...
  private buildSMSNotification(
    ...
    const message = `🚨 CRITICAL FRAUD ALERT: ${alert.alertType}. ${payload.amount ? `Amount: ${payload.currency} ${payload.amount}. ` : ''}Review immediately in Hotel Etuna fraud dashboard.`;
    ...
    try {
      const response = await fetch(url, {
        ...
        headers: {
          ...
          'User-Agent': 'HotelEtuna-FraudDetection/1.0',
        },
        body: JSON.stringify({
