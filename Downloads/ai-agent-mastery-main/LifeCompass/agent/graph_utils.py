...
class GraphitiClient:
    ...
    def __init__(
        ...
        self.llm_api_key = os.getenv("LLM_API_KEY", "ollama")  # Ollama doesn't require API key
        self.llm_choice = os.getenv("LLM_CHOICE", "qwen2.5:14b-instruct")
        
        # Only require API key for non-Ollama providers
        llm_provider = os.getenv("LLM_PROVIDER", "ollama")
        if llm_provider != "ollama" and not self.llm_api_key:
            raise ValueError("LLM_API_KEY environment variable not set")
        
        # Embedding configuration
        self.embedding_base_url = os.getenv("EMBEDDING_BASE_URL", "http://localhost:11434/v1")
        self.embedding_api_key = os.getenv("EMBEDDING_API_KEY", "ollama")  # Ollama doesn't require API key
        self.embedding_model = os.getenv("EMBEDDING_MODEL", "nomic-embed-text")
        self.embedding_dimensions = int(os.getenv("VECTOR_DIMENSION", "768"))

        # Only require API key for non-Ollama providers
        embedding_provider = os.getenv("EMBEDDING_PROVIDER", "ollama")
        if embedding_provider != "ollama" and not self.embedding_api_key:
