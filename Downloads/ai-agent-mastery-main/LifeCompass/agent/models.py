...
class Chunk(BaseModel):
    ...
    def validate_embedding(cls, v: Optional[List[float]]) -> Optional[List[float]]:
        ...
        if v is not None:
            # Accept common embedding dimensions (OpenAI: 1536, Ollama nomic-embed-text: 768, etc.)
            valid_dimensions = {768, 1536, 3072, 384, 512}
            if len(v) not in valid_dimensions:
                raise ValueError(f"Embedding must have one of {valid_dimensions} dimensions, got {len(v)}")
        return v
